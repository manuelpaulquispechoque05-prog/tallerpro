...
<div class="max-w-2xl mx-auto">
    <div class="animate-in glass-card rounded-2xl p-6 lg:p-8">
        <div class="flex items-center gap-3 mb-6">
            ...
            <div>
                <h1 class="text-sm font-semibold text-gray-100">Editar cliente</h1>
                <p class="text-xs text-gray-500 mt-0.5">{{ $cliente->nombre_completo }} — #{{ $cliente->id }}</p>
            </div>
        </div>

        <form method="POST" action="{{ route('panel.clientes.update', $cliente->id) }}">
            @csrf
            @method('PUT')
            @include('panel.clientes.form', ['cliente' => $cliente])
        </form>
    </div>
</div>
@endsection

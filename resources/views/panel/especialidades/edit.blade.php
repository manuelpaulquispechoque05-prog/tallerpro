...
<div class="max-w-lg mx-auto">
    <div class="animate-in glass-card rounded-2xl p-6 lg:p-8">
        ...
        <form method="POST" action="{{ route('panel.especialidades.update', $item->id) }}">
            @csrf @method('PUT')
            <label class="block text-sm font-medium text-gray-300 mb-1.5">Nombre *</label>
            <input name="nombre" type="text" class="w-full px-4 py-3 bg-white/5 border border-white/10 rounded-xl text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50" value="{{ $item->nombre }}" required>

...
<div class="glass-card rounded-2xl overflow-hidden">
    <div class="px-5 lg:px-6 py-4 border-b border-white/[0.06] flex items-center justify-between">
        ...
        <a href="{{ route('panel.sucursales.create') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-blue-600 to-blue-500 text-white text-xs font-semibold rounded-full transition shadow-lg shadow-blue-500/25">Nueva sucursal</a>
    </div>
        ...
        <table class="w-full text-sm">
            ...
            <tbody>
                ...
                <tr class="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                    ...
                    <td class="px-5 py-4 text-sm text-gray-400">{{ $i->direccion }}</td>
                    <td class="px-5 py-4 text-sm text-gray-500">{{ $i->ciudad }}</td>
                    <td class="px-5 py-4 text-sm text-gray-500">{{ $i->telefono ?? '—' }}</td>
                    <td class="px-5 py-4 text-center"><span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium {{ $i->activo ? 'bg-green-500/10 text-green-400' : 'bg-gray-500/10 text-gray-400' }}">{{ $i->activo ? 'Activo' : 'Inactivo' }}</span></td>
                    <td class="px-5 py-4 text-right">
                        <a href="{{ route('panel.sucursales.edit', $i->id) }}" class="p-2 rounded-lg hover:bg-white/10 transition text-gray-400 hover:text-yellow-400 inline-block"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg></a>
                        <form method="POST" action="{{ route('panel.sucursales.destroy', $i->id) }}" onsubmit="return confirm('Eliminar sucursal?')" class="inline">@csrf @method('DELETE')<button type="submit" class="p-2 rounded-lg hover:bg-white/10 transition text-gray-400 hover:text-red-400"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button></form>
                    </td>
                </tr>
                @empty

...
<!-- Alertas de stock -->
<?php $hasAlertas = $alertas['agotados']->count() > 0 || $alertas['bajos']->count() > 0; ?>
<?php if($hasAlertas): ?>
<div class="animate-in glass-card rounded-2xl p-5 lg:p-6 mb-6">
    ...
    <div class="grid sm:grid-cols-2 gap-3">
        <?php $__currentLoopData = $alertas['agotados']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $a): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <div class="flex items-center justify-between p-3 rounded-lg bg-red-500/10 border border-red-500/20">
            <div><span class="text-sm text-gray-200"><?php echo e($a->repuesto?->nombre); ?></span><span class="text-xs text-gray-500 ml-2"><?php echo e($a->repuesto?->codigo); ?></span></div>
            <span class="text-xs font-semibold text-red-400">AGOTADO (0)</span>
        </div>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        <?php $__currentLoopData = $alertas['bajos']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $a): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <div class="flex items-center justify-between p-3 rounded-lg bg-amber-500/10 border border-amber-500/20">
            <div><span class="text-sm text-gray-200"><?php echo e($a->repuesto?->nombre); ?></span><span class="text-xs text-gray-500 ml-2"><?php echo e($a->repuesto?->codigo); ?></span></div>
            <span class="text-xs font-semibold text-amber-400"><?php echo e($a->stock_actual); ?> / <?php echo e($a->stock_minimo); ?> min</span>
        </div>
        <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </div>
</div>
...
<div class="glass-card rounded-2xl overflow-hidden">
    <div class="px-5 lg:px-6 py-4 border-b border-white/[0.06] flex items-center justify-between">
        ...
        <div class="flex gap-2">
            <a href="<?php echo e(route('panel.inventario.ingreso')); ?>" class="inline-flex items-center px-3 py-1.5 bg-gradient-to-r from-blue-600 to-blue-500 text-white text-xs font-semibold rounded-full transition shadow-lg shadow-blue-500/25">
            ...
            <a href="<?php echo e(route('panel.inventario.movimientos')); ?>" class="text-xs font-medium text-blue-400 hover:text-blue-300 transition">Movimientos</a>
        </div>
    </div>
    <div class="px-5 lg:px-6 py-3 border-b border-white/[0.06]">
        <form method="GET" x-data>
            <div class="relative max-w-md">
                ...
                <input type="text" name="busqueda" value="<?php echo e($busqueda); ?>" placeholder="Buscar por repuesto..." class="w-full pl-10 pr-4 py-2 bg-white/5 border border-white/10 rounded-xl text-sm text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500/50" x-on:input.debounce.500ms="$el.form.submit()">
            </div>
        </form>
    </div>
            ...
            <tbody>
                <?php $__empty_1 = true; $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $i): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    ...
                    <tr class="border-b border-white/[0.03] hover:bg-white/[0.02] transition-colors">
                        <td class="px-5 py-4"><span class="text-sm text-gray-200"><?php echo e($i->repuesto?->nombre); ?></span><br><span class="text-xs text-gray-600"><?php echo e($i->repuesto?->codigo); ?></span></td>
                        <td class="px-5 py-4 text-sm text-gray-500"><?php echo e($i->sucursal?->nombre ?? 'Principal'); ?></td>
                        <td class="px-5 py-4 text-right text-sm font-medium <?php echo e($colorAlerta[$alerta]); ?>"><?php echo e($i->stock_actual); ?></td>
                        <td class="px-5 py-4 text-right text-sm text-gray-500"><?php echo e($i->stock_minimo); ?></td>
                        <td class="px-5 py-4 text-center">
                            <span class="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium <?php echo e($badgeParts[0]); ?> <?php echo e($badgeParts[1]); ?>"><?php echo e($badgeParts[2]); ?></span>
                        </td>
                        <td class="px-5 py-4 text-center">
                            <a href="<?php echo e(route('panel.inventario.historial', $i->repuesto_id)); ?>" class="text-xs text-blue-400 hover:text-blue-300 transition">Ver movimientos</a>
                        </td>
                    </tr>
    ...
    <?php if($items->hasPages()): ?>
        <div class="px-5 lg:px-6 py-4 border-t border-white/[0.06]"><?php echo e($items->links()); ?></div>
    <?php endif; ?>
</div>
<?php $__env->stopSection(); ?>

<?php echo $__env->make('layouts.panel', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\laragon\www\TallerPro\resources\views/panel/inventario/index.blade.php ENDPATH**/ ?>
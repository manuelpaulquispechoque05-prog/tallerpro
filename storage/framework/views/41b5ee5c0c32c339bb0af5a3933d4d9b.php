<!DOCTYPE html>
<html lang="<?php echo e(str_replace('_', '-', app()->getLocale())); ?>" x-data="theme()">
<head>
    <meta charset="utf-8">
    <meta name="viewport" content="width=device-width, initial-scale=1">
    <meta name="csrf-token" content="<?php echo e(csrf_token()); ?>">
    <title><?php echo $__env->yieldContent('title', 'Dashboard'); ?> — <?php echo e(config('app.name', 'Taller Pro')); ?></title>
...
<body class="font-sans antialiased bg-[#0a0a0a] text-gray-100">
    ...
    <aside class="fixed top-0 left-0 z-50 h-screen w-64 lg:w-64 sidebar-transition flex flex-col border-r border-white/[0.06] glass-sidebar"
        ...
        <div class="flex items-center justify-between h-16 px-5 border-b border-white/[0.06] shrink-0">
            <a href="<?php echo e(route('panel.dashboard')); ?>" class="flex items-center gap-2.5 group">
        ...
        <nav class="flex-1 overflow-y-auto px-3 py-5 space-y-0.5">
            ...
            <?php $__currentLoopData = $navItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                ...
                ?>
                <a href="<?php echo e($routeName ? route($routeName) : '#'); ?>" 
                   ...
                   :class="$isActive ? 'bg-blue-500/10 text-blue-400 nav-item-active' : 'text-gray-400 hover:text-gray-200 hover:bg-white/[0.04]'">
                    <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="<?php echo e($item['icon']); ?>"/></svg>
            ...
            <div x-data="{ open: false }" class="relative">
                <button @click="open = !open" class="flex items-center gap-3 w-full px-3 py-2.5 rounded-xl text-sm transition-all hover:bg-white/[0.04]">
                    ...
                    <div class="flex-1 text-left">
                        <p class="text-sm font-medium text-gray-200 leading-tight truncate max-w-[120px]"><?php echo e(Auth::user()->name ?? 'Usuario'); ?></p>
                        <p class="text-xs text-gray-500 truncate max-w-[120px]"><?php echo e(Auth::user()->email ?? ''); ?></p>
                    </div>
                ...
                <div x-show="open" @click.away="open = false" x-transition:enter="transition ease-out duration-150" x-transition:enter-start="opacity-0 -translate-y-2" x-transition:enter-end="opacity-100 translate-y-0" class="absolute bottom-full left-0 right-0 mb-2 rounded-xl bg-[#1a1a1a] border border-white/[0.06] shadow-2xl py-1 overflow-hidden">
                            <a href="<?php echo e(route('panel.perfil')); ?>" class="block px-4 py-2.5 text-sm text-gray-300 hover:bg-white/[0.04] transition">Mi perfil</a>
            ...
            <div class="flex items-center justify-between h-16 px-4 lg:px-6">
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex items-center gap-2.5">
                        ...
                        <h1 class="text-sm font-semibold text-gray-100"><?php echo $__env->yieldContent('title', 'Dashboard'); ?></h1>
                    </div>
                </div>
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div x-data="{ open: false }" class="relative hidden sm:block">
                        <button @click="open = !open" @click.away="open = false" class="flex items-center gap-2 pl-3 pr-2 py-1.5 rounded-xl hover:bg-white/[0.04] transition">
                            <div class="w-7 h-7 rounded-full bg-gradient-to-br from-blue-500 to-blue-600 flex items-center justify-center text-white text-xs font-bold"><?php echo e(substr(Auth::user()->name ?? 'U', 0, 1)); ?></div>
...
<?php /**PATH C:\laragon\www\TallerPro\resources\views/layouts/panel.blade.php ENDPATH**/ ?>